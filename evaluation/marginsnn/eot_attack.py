...
import evaluation.marginsnn.adaptive_attack.eot as eot
...
def test(eps, nb_iter, eps_iter):
    ...
    for clndata, target in tqdm(test_loader):
        ...
        advdata = eot.EOT_PGD(model, clndata, target, sample_size=15)
...
if __name__ == '__main__':
    ...
    train_loader, test_loader = data_selection(dataset, normalize=use_normalization, test_batch=1)
    ...
    model_name = 'resnet18_cifar10_256nodenum_0.0003lr_783seed_without_normalization_le_margin_kl.pth'
    model.load_state_dict(torch.load(os.path.join(save_path, model_name), map_location='cpu'))

    model = model.to(device)

    loss_function = margin_loss()
    test(eps, nb_iter, eps_iter)