...
class le_net(nn.Module):
    ...
    def forward(self, x):
        ...
        x_embedding = self.fc1(x_embedding)
...
if __name__ == '__main__':
    ...
    model_name = 'resnet18_mnist_256nodenum_0.0003lr_1003seed_without_normalization_le_margin_kl.pth'

    if 'kl' in model_name:
        model = dul_net(backbone_model, nb_classes, node_num)
    else:
        model = le_net(backbone_model, nb_classes, node_num)
    ...
    for eps in eps_ls:
        eps = pow(2, eps) / 255
        _, _, adv_acc = test(eps, nb_iter, eps_iter)
