...
class margin_loss(nn.Module):
    ...
    def forward(self, mu, logvar, label_embedding, output, target):
        if distance_metric == 'cosine':
            ...
            label_distance = (label_embedding.unsqueeze(1).repeat((1, classes, 1)) - label_embedding.repeat(classes, 1,
                                                                                                            1)).pow(
                2).sum(dim=2)
...
if __name__ == '__main__':
    ...
    iter_ls = np.arange(0, 110, 5)
    adv_ls = []
    print(iter_ls)
    for nb_iter in iter_ls:
        _, adv_acc, _ = test(eps, nb_iter, eps_iter)
        adv_ls.append(adv_acc)
    plt.plot(iter_ls, adv_ls)
    plt.show()