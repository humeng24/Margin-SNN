...
model_s = 'resnet34'              # lenet | resnet | vgg11 | resnet_dul | resnet34
...
def test(eps):
    ...
    return advcorrect / len(test_loader.dataset)
...
if __name__ == '__main__':
    ...
    model_name = 'resnet34_cifar10_256nodenum_0.0003lr_532seed_without_normalization.pth'
    ...
    eps_ls = np.arange(0, 1.05, 0.03)
    advcorrect_ls = []

    for eps in eps_ls:
        adver_correct = test(eps)
        advcorrect_ls.append(adver_correct)

    plt.xlabel('epsilon')
    plt.ylabel('accuracy')
    plt.plot(eps_ls, advcorrect_ls)
    plt.show()